// The logger package is used to log all intercepted stream calls.
// The package exports the `NewLogger` function which sets up a logger with
// the elogrus hook and returns it.
// There's the `WithElasticsearchLogger` function which sets up a `grpc.ServerOption` to intercept streams with
